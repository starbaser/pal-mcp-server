...
class TestIntelligentFallback:
    ...
    def test_prefers_openai_o3_mini_when_available(self):
        """Test that gpt-5.4 is preferred when OpenAI API key is available (based on preference order)"""
        ...
        assert fallback_model == "gpt-5.4"  # BALANCED category prefers gpt-5.4 first
    ...
    def test_prefers_gemini_flash_when_openai_unavailable(self):
        """Test that gemini-3.1-flash-lite-preview is used when only Gemini API key is available"""
    ...
    def test_fallback_when_no_keys_available(self):
        ...
        fallback_model = ModelProviderRegistry.get_preferred_fallback_model()
        assert fallback_model == "gemini-2.5-flash"  # Default fallback (no API keys)
    ...
    def test_auto_mode_conversation_memory_integration(self):
        ...
        with (
            ...
            with patch("utils.model_context.ModelContext") as mock_context_class:
                ...
                # Verify that ModelContext was called with gpt-5.4 (the intelligent fallback based on preference order)
